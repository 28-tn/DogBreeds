...
dogAnalyzer = Sequential()
dogAnalyzer.add(GlobalAveragePooling2D(input_shape=train_resnet50.shape[1:]))
dogAnalyzer.add(BatchNormalization())
dogAnalyzer.add(Dense(133, activation='softmax'))

#dogAnalyzer.summary()
dogAnalyzer.compile(loss='categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])
dogAnalyzer.load_weights('saved_models/weights.best.Resnet50.hdf5')
